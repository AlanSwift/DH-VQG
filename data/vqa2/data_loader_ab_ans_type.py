...
class VQA2Dataset(data.Dataset):
    def __init__(self, split_dic_path, vocab_path, answer_path, ans_type_path, image_size, image_crop_size,
                 prop_thresh=0, pad_length=20, verbase=1, ppl_num=100, split="train", ratio=1):
        super(VQA2Dataset, self).__init__()

        with open(ans_type_path, "r") as f:
            ans_type = json.load(f)
        self.ans2cls = {}
        cls_list = list(ans_type.keys())
        for cls, ans_list in ans_type.items():
            for ans in ans_list:
                self.ans2cls[ans] = cls
        ...
        for cls in cls_list:
            if self.vocab.word2idx.get(cls) is None:
    ...
    def __getitem__(self, item):
        ...
        ans_type = self.ans2cls.get(inst["answer"], "other")
        answer_type_ids = self.vocab.convert_tokens([ans_type], [ans_type])
        answer_type_ids = np.array(answer_type_ids)
        ...
        img, box_feats, box_info, question, answer, answer_type, visual_hint = self.vectorize(img=img, box_feats=box_feats,
                                                                                 box_infos=box_infos,
                                                                                 box_cls=box_cls, question=question_ids,
                                                                                 answer=answer_ids,
                                                                                 answer_type=answer_type_ids,
                                                                                 visual_hint=visual_hint)
        return img, box_feats, box_info, visual_hint, question, answer, answer_gt_idx, answer_type, question_str, question_idx

    def __len__(self):
        return int(len(self.split_idx2dic)*self.ratio)

    def vectorize(self, img, box_feats, box_infos, box_cls, question, answer, answer_type, visual_hint):
        box_feats = torch.from_numpy(box_feats)
        box_infos = torch.from_numpy(box_infos)
        box_cls = torch.from_numpy(box_cls)
        question = torch.from_numpy(question)
        answer = torch.from_numpy(answer)
        answer_type = torch.from_numpy(answer_type)
        ...
        return img, box_feats, box_info, question, answer, answer_type, visual_hint


if __name__ == "__main__":
    dataset = VQA2Dataset("/home/shiina/data/aaai/vqa2/train_split_dic.pkl",
