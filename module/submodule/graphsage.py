...
class GraphSAGE(GNNBase):
    ...
    def forward(self, node_feats, adj_batch):
        dgl_graph_list = []
        ...
        bg = dgl.batch(dgl_graph_list)
        b = node_feats.shape[0]
        step = node_feats.shape[1]
        assert len(bg.nodes) == node_feats.shape[0] * node_feats.shape[1]
        node_num = len(bg.nodes)
        h = node_feats.view(node_num, -1) #get the node feature tensor from graph
        g = bg
        edge_weight = bg.edata['edge_weight']
        ...
        return logits.view(b, step, self.d_out)
